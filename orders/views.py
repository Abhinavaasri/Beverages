...
from .models import Category, RegularPizza, SicilianPizza, Toppings, Sub, Pasta, Salad, Drink, DinnerPlatters, UserOrder, SavedCarts, Contact
...
def find(request):
    if request.user.is_authenticated:
        return render(request, "orders/find.html")
    else:
        return redirect("orders:login")
...
def contact(request):
    if request.method == 'POST':
        ...
        return render(request, 'orders/contact.html', {'show_popup': True})
    return render(request, 'orders/contact.html')
